...
import { ChevronRight, LogOut, MapPin, ShoppingBag, User } from "lucide-react";
...
export function BuyerAccountSidebarNav() {
	...
	return (
		<aside className="w-full md:w-64 shrink-0 border-r border-border/60 bg-card p-4 flex flex-col justify-between rounded-xl md:rounded-r-none">
			...
			<div className="flex flex-col gap-2 pt-4 border-t border-border/40 mt-6 md:mt-0">
				<button
					type="button"
					onClick={() => signOut()}
					className="flex items-center gap-2 text-xs font-medium text-destructive hover:bg-destructive/10 px-2.5 py-2 rounded-md transition-colors w-full text-left"
				>
