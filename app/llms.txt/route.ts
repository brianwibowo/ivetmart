	...
	try {
		...
		const [products, collections, legalPages, posts] = await Promise.all([
			commerce
				...
				.catch(() => ({ data: [] as never[] })),
			commerce
				.collectionBrowse({ active: true, limit: FEATURED_COLLECTIONS })
				.catch(() => ({ data: [] as never[] })),
			commerce.legalPageBrowse().catch(() => ({ data: [] as never[] })),
			blogEnabled
				? commerce.postBrowse({ active: true, limit: FEATURED_POSTS }).catch(() => ({ data: [] as never[] }))
		...
		if (legalPages.data.length > 0) {
			...
			sections.push("");
			legalPages.data.map((p) => sections.push(`- [${p.label}](${baseUrl}/legal${p.href})`));
			sections.push("");
		}
		...
		return new Response("# Ivet Mart\n\nllms.txt is temporarily unavailable.", {
			headers: {
				"Content-Type": "text/plain; charset=utf-8",
